...
**문제**
초기 렌더 시 로딩 시간이 과도하게 길어지고, 특정 환경에서 로딩 스피너만 지속되는 현상 발생
**원인**
- 현재 위치 기반 날씨 조회 시 geolocation API 응답 지연
- 최근 Wi-Fi 환경 변경으로 위치 정보 획득 시간이 비정상적으로 증가
**해결**
- 일정 시간(3초) 내 위치 정보를 가져오지 못할 경우 기본 도시(Seoul)로 fallback 처리
- setTimeout을 활용해 초기 로딩 지연 UX 개선
**결과**
- 초기 진입 시 로딩 시간 안정화
- 네트워크 환경에 관계없이 일관된 사용자 경험 확보
